...
void solver::BDF4Step()
{
    ...
    if (cRank==0)
    {
        ...
        //MPI_Sendrecv(Fields.data, 1, BD4Type[0], 0, 0, odetempField2.data, iterPoints, MPI_LONG_DOUBLE, 0, 0, MPI_COMM_WORLD,& status);
    ...
    {
        MPI_Recv(odetempField2.data, iterPoints, MPI_LONG_DOUBLE, 0, cRank, MPI_COMM_WORLD,& status);
    }
    odetempField3=odetempField2;
    while (1)
    {
        Fun(Fields);
        if (cRank==0)
        {
            ...
            //MPI_Sendrecv(Fields.data, 1, BD4Type[0], 0, 100, iterFieldsLocal.data, iterPoints, MPI_LONG_DOUBLE, 0, 100, MPI_COMM_WORLD,& status);
            for (int iterf=0; iterf<NumField; ++iterf)
            {
                for (int iter=0; iter<bossP; ++iter)
                {
        ...
        {
            MPI_Recv(iterFieldsLocal.data, iterPoints, MPI_LONG_DOUBLE, 0, 100+cRank, MPI_COMM_WORLD,& status);
        }
        ...
        for (int iter=0; iter<iterPoints; ++iter)
        {
            ...
            if (error<odetempField3[iter])
            {
                error=odetempField3[iter];
            }
        }
        //error=gsl_matrix_max(odetempField3);
        MPI_Allreduce(&error, &err, 1, MPI_LONG_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
        if (cRank!=0)
            MPI_Ssend(iterFieldsLocal.data, iterPoints, MPI_LONG_DOUBLE, 0, 200+cRank, MPI_COMM_WORLD);
        else
        {
            //MPI_Sendrecv(iterFieldsLocal.data, iterPoints, MPI_LONG_DOUBLE, 0, 200,Fields.data, 1, BD4Type[0], 0, 200, MPI_COMM_WORLD,& status);
            for (int iterf=0; iterf<NumField; ++iterf)
            {
                for (int iter=0; iter<bossP; ++iter)
                {
